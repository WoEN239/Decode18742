...
internal object TURRET_CONFIG {
    ...
    @JvmField
    var VELOCITY_PULLEY_COMPENSATE_K = 0.8
    ...
    @JvmField
    var SHORT_FAR_ANGLE_POSITION = 0.43

    @JvmField
    var SHORT_FAR_DISTANCE = 2.63

    @JvmField
    var CLOSE_DISTANCE = 1.0
...
fun attachTurret(collector: Collector) {
    ...
    collector.updateEvent += {
        ...
        val l = clamp(basketErrPulley.length(), 1.0, 2.71)
        ...
        if (odometry.orientation.x > 0.5) {
            targetPulleyVelocity = (TURRET_CONFIG.CLOSE_PULLEY_VELOCITY + clamp(
                (l - TURRET_CONFIG.CLOSE_DISTANCE) /
                        (TURRET_CONFIG.SHORT_FAR_DISTANCE - TURRET_CONFIG.CLOSE_DISTANCE), 0.0, 1.0
            ...
            targetPulleyVelocity = (TURRET_CONFIG.LONG_CLOSE_PULLEY_VELOCITY + clamp(
                (l - TURRET_CONFIG.LONG_CLOSE_DISTANCE) /
                        (TURRET_CONFIG.FAR_DISTANCE - TURRET_CONFIG.LONG_CLOSE_DISTANCE), 0.0, 1.0
        ...
        val basketErrHeading =
            GameSettings.startOrientation.basketPosition.invoke() - (odometry.orientation.pos + TURRET_CONFIG.TURRET_CENTER_POS
                .turn(odometry.orientation.angle))
